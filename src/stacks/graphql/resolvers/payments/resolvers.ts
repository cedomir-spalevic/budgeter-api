...
const resolvers = {
   payments: async (args: Record<string, unknown>, context: BudgeterRequestAuth): Promise<PublicPayment[]> => {
      const queryStringParameters: GetListQueryStringParameters = {
         skip: args["skip"] as number,
         limit: args["limit"] as number
      }
      const paymentsProcessor = await PaymentsProcessor.getInstance(context.userId);
      return paymentsProcessor.get(queryStringParameters);
   },
   paymentById: async (args: Record<string, unknown>, context: BudgeterRequestAuth): Promise<PublicPayment> => {
      const paymentId = args["paymentId"] as string;
      const paymentsProcessor = await PaymentsProcessor.getInstance(context.userId);
      return paymentsProcessor.getById(new ObjectId(paymentId));
   },
   createPayment: async (args: Record<string, unknown>, context: BudgeterRequestAuth): Promise<PublicPayment> => {
   ...
   updatePayment: async (args: Record<string, unknown>, context: BudgeterRequestAuth): Promise<PublicPayment> => {
      const paymentId = args["paymentId"] as string;
   ...
   deletePayment: async (args: Record<string, unknown>, context: BudgeterRequestAuth): Promise<ObjectId> => {
      const paymentId = args["paymentId"] as string;
      const paymentsProcessor = await PaymentsProcessor.getInstance(context.userId);
