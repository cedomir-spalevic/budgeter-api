 ...
 * @param year 
 */
export const getWeeklyOccurrenceLength = (dayOfWeek: number, month: number, year: number) => {
   let counter = 0;
   let monthToUse = new Date(year, month, 1);
 ...
 * @param year 
 */
export const getBiweeklyOccurrenceLength = (dayOfWeek: number, month: number, year: number) => {
   let counter = 0;
   let monthToUse = new Date(year, month, 1);
   while (monthToUse.getMonth() === month) {
      if (monthToUse.getDay() === dayOfWeek) {
