 ...
 */
export const generateAccessToken = (userId: string, refreshToken: string): { token: string, expires: number } => {
   const now = Date.now();
   ...
   const token = jwt.sign(payload, process.env.JWT_KEY, { algorithm: "HS256", expiresIn: "15 mins" })
...
export const decodeAccessToken = (token: string): Token => {
   try {
      return jwt.verify(token, process.env.JWT_KEY, { algorithms: ["HS256"] }) as Token;
   }
   catch (error) {
